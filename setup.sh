...
then
	...
	tmux send-keys -t tfm:0.2 "" Enter # WRITE USER KEY !CAREFUL¡
...
then
	...
	tmux send-keys -t tfm:1.0 "" Enter # WRITE USER KEY !CAREFUL¡
	tmux send-keys -t tfm:1.1 "sudo -u sawtooth settings-tp -v" Enter
	tmux send-keys -t tfm:1.1 "" Enter # WRITE USER KEY !CAREFUL¡
	tmux send-keys -t tfm:1.2 "sudo -u sawtooth sawtooth-rest-api -vv" Enter
	tmux send-keys -t tfm:1.2 "" Enter # WRITE USER KEY !CAREFUL¡
	if [[ $2 = "bft" ]];
	then
